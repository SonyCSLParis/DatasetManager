...
class PianoDataset(MusicDataset):
    """
    ...
    def make_tensor_dataset(self, frame_orchestra=None):
        ...
        # Iterate over files
        for score_id, score in tqdm(enumerate(self.iterator_gen())):
            ############################################################
            # Compute original pianoroll
            pianoroll, onsets, _ = score_to_pianoroll(
                score=score,
                subdivision=self.subdivision,
                simplify_instrumentation=None,
                instrument_grouping=None,
                transpose_to_sounding_pitch=True,
                integrate_discretization=self.integrate_discretization,
                binarize=False,
            )

            raise NotImplementedError('Ca na aucun sens la puianoroll represnetation, vraiment')
            pianoroll = quantize_velocity_pianoroll_frame(pianoroll["Piano"], self.velocity_quantization)
            ...
            total_chunk_counter, too_many_instruments_frame, impossible_transposition = \
                self.transpose_loop_iteration(pianoroll, onsets, transposition_semi_tone,
                                              minimum_transpositions_allowed, maximum_transpositions_allowed,
